...
double Options::getDouble(const std::string& optionName) {
    return std::stod(getString(optionName));
}
...
int Options::getInt(const std::string& optionName) {
    return std::stol(getString(optionName));
}
...
void Options::setOptions(std::vector<std::string> options) {
   m_processedQ = 0;

   m_extraArgv.resize(options.size());
   m_extraArgv_strings.resize(options.size());
   int oldsize = 0;

   int i;
   for (i=0; i<options.size(); i++) {
      m_extraArgv_strings[i+oldsize] = options[i];
...
void Options::appendOptions(std::vector<std::string> options) {
   m_processedQ = 0;
   ...
   m_extraArgv.resize(oldsize + options.size());
   m_extraArgv_strings.resize(oldsize + options.size());
...
void Options::process(std::vector<std::string> options, int error_check, int suppress) {
    setOptions(options);
...
void Options::xverify(std::vector<std::string> options, int error_check, int suppress) {
   setOptions(options);
   xverify(error_check, suppress);
}
